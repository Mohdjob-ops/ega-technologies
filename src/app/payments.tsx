...
type ChapaReturnParams = {
  ...
  paypal_cancelled?: string | string[];
};
...
export default function PaymentsScreen() {
  ...
  const [paypalInitializing, setPaypalInitializing] =
    useState(false);
  const [paypalCapturing, setPaypalCapturing] =
    useState(false);
  ...
  const processedPayPalOrder = useRef("");
  ...
  async function startPayPalPayment() {
    ...
    setMessage("🔄 Preparing secure PayPal checkout...");
    ...
    try {
      ...
      if (error) {
        setMessage("❌ PayPal initialization error: " + error.message);
        return;
      }

      if (!data?.success || !data?.approval_url) {
        setMessage("❌ " + (data?.message || "PayPal could not initialize the payment"));
        return;
      }
  ...
  async function capturePayPalPayment(orderId: string, studentId: string) {
    if (!orderId || !studentId) return;

    setPaypalCapturing(true);
    setMessage("🔄 Confirming and capturing your PayPal payment...");

    try {
      const { data, error } = await supabase.functions.invoke(
        "capture-paypal-payment",
        { body: { order_id: orderId, student_id: studentId } }
      );

      if (error) {
        setMessage("❌ PayPal capture error: " + error.message);
        return;
      }
      ...
      if (!data?.success || !data?.verified) {
        setMessage(
          "⚠️ " +
            (data?.message || "PayPal did not confirm a completed payment. No student record was changed.")
        );
        return;
      }
      ...
      if (data.student) {
        ...
        setSearchValue(data.student.student_id || studentId);
      } else {
        await findStudent(studentId, false);
      }

      await loadPaymentHistory(studentId);
      setMessage(
        data.already_verified
          ? "✅ This PayPal payment was already verified; no duplicate was created."
          : `✅ PayPal payment verified: ${formatMoney(data.payment?.amount)} ${data.payment?.currency || "USD"}`
      );
    } catch (error) {
      setMessage(
  ...
  function selectPaymentMethod(
    ...
    if (method === "paypal") {
      setMessage(
        "✅ PayPal selected. Press Continue with PayPal to open secure checkout."
      );
      return;
    }
  ...
  async function continueSelectedPayment() {
    ...
    if (selectedPaymentMethod === "paypal") {
      setManualDebug("➡️ Starting PayPal payment");
  ...
  useEffect(() => {
    const orderId = getParam(params.token);
    const returnedStudentId = getParam(params.student_id);
    const cancelled = getParam(params.paypal_cancelled);

    if (cancelled) {
      setMessage("⚠️ PayPal checkout was cancelled. No payment was recorded.");
      return;
    }

    if (!orderId || !returnedStudentId || processedPayPalOrder.current === orderId) {
      return;
    }

    processedPayPalOrder.current = orderId;
    setSearchValue(returnedStudentId);
    capturePayPalPayment(orderId, returnedStudentId);
  }, [params.token, params.PayerID, params.paypal_cancelled, params.student_id]);
            ...
            {!isPaid && (
              <View style={styles.paymentMethodsBox}>
                ...
                <View style={styles.selectedMethodInfo}>
                  ...
                  <Text style={styles.selectedMethodDescription}>
                    {selectedPaymentMethod === "chapa"
                      ...
                      : selectedPaymentMethod === "paypal"
                        ? "You will be redirected to PayPal. The USD checkout amount is calculated from the ETB balance using the configured server rate."
                        : selectedPaymentMethod === "bank"
                ...
                >
                  {(initializing &&
                    selectedPaymentMethod === "chapa") ||
                  ((paypalInitializing || paypalCapturing) &&
                    selectedPaymentMethod === "paypal") ||
                  (submittingManual &&
                    (selectedPaymentMethod === "bank" ||
                      selectedPaymentMethod === "cash")) ? (
